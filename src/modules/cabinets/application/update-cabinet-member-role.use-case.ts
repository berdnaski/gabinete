...
export class UpdateCabinetMemberRoleUseCase {
  ...
  async execute(
    input: UpdateCabinetMemberRoleInput,
    ...
    const requesterMembership = await this.membersRepository.findMembership(
      input.requesterId,
      cabinet.id,
    );
    if (
      !requesterMembership ||
      requesterMembership.role !== CabinetRole.OWNER
    ) {
      throw new ForbiddenException(
        'Apenas proprietários de gabinetes podem atualizar as funções dos membros',
      );
    }

    if (input.targetUserId === input.requesterId) {
      throw new BadRequestException(
        'Você não pode alterar seu próprio cargo. Em vez disso, transfira a propriedade.',
      );
    }

    const targetMembership = await this.membersRepository.findMembership(
      input.targetUserId,
      cabinet.id,
    );
    if (!targetMembership) {
