...
import { PaginatedResponse, PaginationParams } from '../../../shared/domain/pagination.interface';
...
export class ListCategoriesUseCase {
  ...
  async execute(params: PaginationParams): Promise<PaginatedResponse<CategoryEntity>> {
    const { page, limit } = PaginationHelper.getSkipTake(params);
    const { items, total } = await this.categoriesRepository.list({ page, limit });
